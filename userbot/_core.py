...
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {AuraX}\n\n⚡ **[ßååþ ßðl†ê Äµråxßð†](t.me/AuraXUserbot)** ⚡"
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {AuraX}\n\n{string}\n\n        ⚡ **[ßååþ ßðl†ê Äµråxßð†](t.me/AuraXUserbot)** ⚡", link_preview=False)
...
async def uninstall(AuraX):
    if AuraX.fwd_from:
        return
    shortname = AuraX.pattern_match["shortname"]
    dir_path =f"./userbot/plugins/{shortname}.py"
    try:
        remove_plugin(shortname)
        os.remove(dir_path)
        await AuraX.edit(f"Uninstalled `{shortname}` successfully")
    except OSError as e:
        await AuraX.edit("Error: %s : %s" % (dir_path, e.strerror))
