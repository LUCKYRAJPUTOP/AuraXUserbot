...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            AuraX = await event.client.send_file(
                event.chat_id,
                AuraXgif,
            ...
            await event.client(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=AuraX.media.document.id,
                        access_hash=AuraX.media.document.access_hash,
                        file_reference=AuraX.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
