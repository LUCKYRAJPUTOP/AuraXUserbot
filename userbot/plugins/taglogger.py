...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        AuraX = await event.client.get_entity(event.sender_id)
        if AuraX.bot or AuraX.verified or AuraX.support:
            return

        AuraXm = f"[{get_display_name(AuraX)}](tg://user?id={AuraX.id})"
