...
from AuraXBot import CmdHelp, bot as AuraXBot, ALIVE_NAME
...
AuraX = f"[{DEFAULTUSER}](tg://user?id={AuraX})"

@AuraXBot.on(admin_cmd(pattern="hvd$"))
@AuraXBot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@AuraXBot.on(admin_cmd(pattern="vday$"))
@AuraXBot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
