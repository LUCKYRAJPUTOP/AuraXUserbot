...
AuraX_NAME = str(ALIVE_NAME) if ALIVE_NAME else "Äµråxßð†"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await edit_or_reply(event, 
                   "✓ **File uploaded to [telegraph](https://telegra.ph{})** \n✓ **Time Taken :-** `{}` secs \n✓ **By :- [{}](tg://user?id={})**".format(
                        media_urls[0], (ms + ms_two), AuraX_NAME, AuraX
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            AuraXboy = f"https://telegra.ph/{response['path']}"
            await edit_or_reply(event, 
                  f"✓ **Pasted to** [telegraph]({AuraXboy}) \n✓ **Time Taken :-** `{ms}` secs\n✓** By :- **[{AuraX_NAME}](tg://user?id={AuraX})", link_preview=True)
    else:
        await edit_or_reply(event, 
