...
# modified and added more tweets by @AuraX_Owner for AuraXBot.....
# added sudo support by @AuraX_Owner
# family completed.....
...
async def nope(AuraX):
    AuraX = AuraX.pattern_match.group(1)
    if not AuraX:
        if AuraX.is_reply:
            (await AuraX.get_reply_message()).message
        else:
            await edit_or_reply(AuraX, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(AuraX))}")
    await tweeter[0].click(
        AuraX.chat_id,
        reply_to=AuraX.reply_to_msg_id,
        silent=True if AuraX.is_reply else False,
        hide_via=True,
    )
    await AuraX.delete()
