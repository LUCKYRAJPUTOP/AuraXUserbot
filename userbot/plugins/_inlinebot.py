#    Copyright (C) @AuraX_Owner 2021-2022
...
#
#
#    This Inline Helper Code is solely owned by @AuraX_Owner
    ...
    buttons.append(
        [
            custom.Button.inline(
               f"◀️ Back {AuraX_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"•{AuraX_emoji} ❌ {AuraX_emoji}•", data="close"
            ),
            custom.Button.inline(
               f"{AuraX_emoji} Next ▶️", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "@AuraXUserbot":
            ...
            result = builder.article(
                "@AuraXUserbot",
                text="""**Hey! This is [Äµråxßð†.](https://t.me/AuraXUserbot) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                    ...
                    [
                        custom.Button.url(
                            "⚡ REPO ⚡", "https://github.com/AuraXNetwork/AuraXBot"),
                        custom.Button.url
                    (
                            "⚡ CREATER ⚡", "https://t.me/AuraX_Owner"
                    )
                    ],
                ],
                link_preview=False,
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "Hoo gya aapka. Kabse tapar tapar dabae jaa rhe h. Khudka bna lo na agr chaiye to. © Äµråxßð†™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Baap Bolte** [Äµråxßð†](https://t.me/AuraXUserbot) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await delete_AuraX(event,
              "⚡Äµråxßð† Menu Provider Is now Closed⚡\n\n         **[©Äµråxßð†™](t.me/AuraXUserbot)**", 5, link_preview=False
            )
        else:
            AuraX_alert = "Ho gya aapka? Kabse tapar tapar dabae jaa rhe h. Khudka bna lo na agr chaiye to. ©Äµråxßð†™"
